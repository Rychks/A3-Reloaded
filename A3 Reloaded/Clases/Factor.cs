...
namespace A3_Reloaded.Clases
{
    public class Factor
    {
        public string remover_factor(string ID,string ID_Item)
        {
            //
            var msg = "";
            try
            {
                ...
                {
                    SqlCommand cmd = new SqlCommand("eliminar_item_factor", conn);
        ...
        public string registrar_Factor(string Titulo, string Descripcion)
        {
            var msg = "";
            try
            {
                ...
                using (var conn = new SqlConnection(constr))
                {
                    SqlCommand cmd = new SqlCommand("insert_factor", conn);
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public string Modificar_factor(int ID, string Titulo, string Descripcion)
        {
            var msg = "";
            try
            {
                ...
                using (var conn = new SqlConnection(constr))
                {
                    SqlCommand cmd = new SqlCommand("update_factor", conn);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@ID", SqlDbType.Int).Value = ID;
                    cmd.Parameters.Add("@Titulo", SqlDbType.VarChar).Value = Titulo;
                    cmd.Parameters.Add("@Descripcion", SqlDbType.VarChar).Value = Descripcion;
        ...
        public DataTable obtener_factor_ID(int ID)
        {
            ...
            try
            {
                using (con)
                {
                    SqlCommand cmd = new SqlCommand("get_factor_id", con);
                    cmd.Parameters.Add("@ID", SqlDbType.Int).Value = ID;
                    cmd.CommandType = CommandType.StoredProcedure;
                    SqlDataAdapter adp = new SqlDataAdapter(cmd);
                    adp.Fill(dt);
