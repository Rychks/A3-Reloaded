...
namespace A3_Reloaded.Clases
{
    ...
    {
        public int obtener_TotalPag_Departamentos(string Nombre, string Activo, int PageSize)
        {
            ...
            try
            {
                ...
                sql.CommandText = "select dbo.get_TotalPag_Departamentos(" + (Nombre == "" || Nombre == null ? (object)DBNull.Value + "null" : ("'" + Nombre + "'")) +
        ...
        public DataTable mostrar_Departamentos(string Nombre, string Activo, int PageIndex, int PageSize)
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["BD_Base"].ConnectionString);
            var msg = "";
        ...
        public string registrar_Departamento(string Nombre, int Activo)
        {
            var msg = "";
            try
            {
                ...
                {
                    SqlCommand cmd = new SqlCommand("insert_Departamento", conn);
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public DataTable obtener_Departamento_ID(int ID)
        {
            var msg = "";
            var constr = ConfigurationManager.ConnectionStrings["BD_Base"].ConnectionString;
            DataTable dt = new DataTable();
            try
            {
                using (var conn = new SqlConnection(constr))
                {
                    SqlCommand cmd = new SqlCommand("get_Departamento_ID", conn);
        ...
        public string actualizar_Departamento(int ID, string Nombre, int Activo)
        {
            var msg = "";
            try
            {
                ...
                using (var conn = new SqlConnection(constr))
                {
                    SqlCommand cmd = new SqlCommand("update_Departamento", conn);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@ID", SqlDbType.Int).Value = ID;
                    cmd.Parameters.Add("@Nombre", SqlDbType.VarChar).Value = Nombre;
                    cmd.Parameters.Add("@Activo", SqlDbType.Int).Value = Activo;
