...
namespace A3_Reloaded.Clases
{
    public class Nota
    {
        public string remover_nota(string ID,string ID_Item)
        {
            var msg = "";
            try
            {
                ...
                {
                    SqlCommand cmd = new SqlCommand("eliminar_item_nota", conn);
        ...
        public string registrar_Nota(string Titulo, string Descripcion)
        {
            ...
            try
            {
                ...
                using (var conn = new SqlConnection(constr))
                {
                    SqlCommand cmd = new SqlCommand("insert_notas", conn);
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public string Modificar_Nota(int ID, string Titulo, string Descripcion)
        {
            var msg = "";
            try
            {
                ...
                using (var conn = new SqlConnection(constr))
                {
                    SqlCommand cmd = new SqlCommand("update_notas", conn);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@ID", SqlDbType.Int).Value = ID;
                    cmd.Parameters.Add("@Titulo", SqlDbType.VarChar).Value = Titulo;
                    cmd.Parameters.Add("@Descripcion", SqlDbType.VarChar).Value = Descripcion;
        ...
        public DataTable obtener_nota_ID(int ID)
        {
            ...
            try
            {
                using (con)
                {
                    SqlCommand cmd = new SqlCommand("get_nota_id", con);
                    cmd.Parameters.Add("@ID", SqlDbType.Int).Value = ID;
                    cmd.CommandType = CommandType.StoredProcedure;
                    SqlDataAdapter adp = new SqlDataAdapter(cmd);
                    adp.Fill(dt);
