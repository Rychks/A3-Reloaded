...
namespace A3_Reloaded.Clases
{
    ...
    {
        public string remover_instruccion(string ID,string ID_Item)
        {
            var msg = "";
            try
            {
                ...
                {
                    SqlCommand cmd = new SqlCommand("eliminar_item_instruccion", conn);
        ...
        public string registrar_Instruccion(string Titulo, string Descripcion)
        {
            var msg = "";
            try
            {
                ...
                using (var conn = new SqlConnection(constr))
                {
                    SqlCommand cmd = new SqlCommand("insert_instrucciones", conn);
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public string Modificar_Instruccion(int ID, string Titulo, string Descripcion)
        {
            var msg = "";
            try
            {
                ...
                using (var conn = new SqlConnection(constr))
                {
                    SqlCommand cmd = new SqlCommand("update_instrucciones", conn);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@ID", SqlDbType.Int).Value = ID;
                    cmd.Parameters.Add("@Titulo", SqlDbType.VarChar).Value = Titulo;
                    cmd.Parameters.Add("@Descripcion", SqlDbType.VarChar).Value = Descripcion;
        ...
        public DataTable obtener_Instruccion_ID(int ID)
        {
            ...
            try
            {
                using (con)
                {
                    SqlCommand cmd = new SqlCommand("get_instruccion_id", con);
                    cmd.Parameters.Add("@ID", SqlDbType.Int).Value = ID;
                    cmd.CommandType = CommandType.StoredProcedure;
                    SqlDataAdapter adp = new SqlDataAdapter(cmd);
                    adp.Fill(dt);
